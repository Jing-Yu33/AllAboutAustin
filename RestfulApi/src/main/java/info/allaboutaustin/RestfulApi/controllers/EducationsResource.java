...
public class EducationsResource {
	...
	@GetMapping("")
	public HashMap<String,SchoolData> getAllFoods(){
		List<Zipcode> zipcodes = ZipcodeRepo.findAll();
		
		HashMap<String,SchoolData> map = new HashMap<String,SchoolData>();
		for(Zipcode zc : zipcodes) {
			map.put(zc.getZipcode(), zc.getEducationData());
		}
